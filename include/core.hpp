/*************************************************************************
  Adotex-ba, a fast bundle adjustment framework enhanced by Adotex.
...
public:
	...
	/**
	 * \brief Holds the external paramters between the camera and the object.
	...
	decltype(auto) reprojection_error_xy() const noexcept {
		return _Myerrxy;
	}
	std::string report() const noexcept;

	std::vector<point3d_t> refined_object_points() const noexcept;

private:
	value_t _Myerror;
	std::vector<point2d_t> _Myerrxy;
...
public:
	...
	struct camera_type
	{
		mutable array_1x6_t left;  //fx, fy, cx, cy, k1, k2
		mutable array_1x6_t right; //fx, fy, cx, cy, k1, k2
	 ...
	 * \param 'point_correspondences' input observed stereo correspondences, \sa "correspondence_type".
	 * \param 'options' config adotex solver properties. \sa options_type 
	 */
	 ...
	 * \param 'cameras' initial internal calibration paramters for both cameras, \sa "camera_type".
	 * \param 'geometry' initial vision geometry including relative rotation and translation, \st geometry = {rx, ry, rz, tx, ty, tx}.
	 */
	...
	// Following the properties below to get results, \sa each method name...
